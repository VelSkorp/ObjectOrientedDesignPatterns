...
In most cases, *extrinsic state* gets moved to the container object, which aggregates objects before we apply the pattern.

Solution is to create a separate context class that would store the extrinsic state along with reference to the flyweight object. This approach would require having just a single array in the container class.
